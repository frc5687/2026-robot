...
private:
  ...
  typename Map::mapped_type &GetOrCreate(const std::string &name, Map &map, Factory &&make) {
    // Try read-only first (shared lock — multiple readers, no blocking)
    {
      std::shared_lock rl(m_lock);
      if (auto it = map.find(name); it != map.end())
        return it->second;
    }
    // Miss — take exclusive lock and double-check
