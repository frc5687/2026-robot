...
void SimHoodIO::UpdateInputs(HoodIOInputs& inputs){
    ...
    inputs.leftHoodRotation = (hoodPosition * kGearRatio) /
        (2.0 * std::numbers::pi * 1_rad) * 1_tr;

    inputs.rightHoodRotation = (hoodPosition * kGearRatio) /
        (2.0 * std::numbers::pi * 1_rad) * 1_tr;

    inputs.leftHoodVelocity = (hoodVelocity * kGearRatio) /
        (2.0 * std::numbers::pi * 1_rad) * 1_tr;

    inputs.rightHoodVelocity = (hoodVelocity * kGearRatio) /
        (2.0 * std::numbers::pi * 1_rad) * 1_tr;
}
...
void SimHoodIO::SetHoodPosition(units::turn_t leftHoodPosition, units::turn_t rightHoodPosition) {
