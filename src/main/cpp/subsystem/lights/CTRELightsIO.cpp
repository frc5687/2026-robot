...
CTRELightsIO::CTRELightsIO(const CANDevice &candle)
    : m_candle(candle.id, candle.bus), m_colorRequest(0, 355),
      m_colorBlink(0, 355), m_alliance(frc::DriverStation::kRed),
      m_debouncer(0.1_s) {
...
void CTRELightsIO::UpdateInputs(LightsIOInputs &inputs) {
  ...
  m_debouncedBlink ? SetBlinkColor(GetActiveColor())
                   : SetSolidColor(GetActiveColor());
}
...
LEDColor CTRELightsIO::GetActiveColor() {
  ...
  if (MatchTracker::Instance().CanAllianceScore(m_alliance)) {
  ...
  } else if (MatchTracker::Instance().ShouldPrespinForAlliance(m_alliance,
                                                               3_s)) {
    m_blinkOn = true;
    return LEDColor::Green;
  } else if (MatchTracker::Instance().ShouldPrespinForAlliance(m_alliance,
    ...
    m_blinkOn = false;
    return frc::DriverStation::kBlue == m_alliance ? LEDColor::Blue
                                                   : LEDColor::Red;
  }
}
...
void CTRELightsIO::SetBlinkColor(LEDColor color) {
  m_currentColor = ToRGBW(color);
  m_candle.SetControl(
      m_colorBlink.WithColor(m_currentColor).WithFrameRate(units::hertz_t{5}));
}
