...
frc::Transform3d TransformFromArray(const std::vector<double>& arr) {
  if (arr.size() < 6) return frc::Transform3d{};
...
AprilTagObservation ObservationFromFiducial(
    const LimelightHelpers::FiducialResultClass& fid,
...
const AprilTagObservation* BestObservation(
...
void LimelightVisionIO::SetRobotOrientation(units::degree_t yaw, units::degrees_per_second_t yawRate,
  ...
  for (const auto& name : m_limelightNames) {
    LimelightHelpers::SetRobotOrientation_NoFlush(name, yaw.value(), yawRate.value(), pitch.value(),
...
VisionMeasurement LimelightVisionIO::MeasurementFromEstimate(
  ...
  m.ambiguity      = est.rawFiducials.empty() ? 0.0
                                              : est.rawFiducials[0].ambiguity;
...
void LimelightVisionIO::ProcessCamera(const std::string& name,
  ...
  // Per-tag observations from JSON, saw large loop times with json, so maybe avoid
  ...
  if (inputs.visionPoseMeasurements.count(name)) {
    ...
    const double latSec =
        (results.targetingResults.m_latencyPipeline +
