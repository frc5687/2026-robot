...
void CTRETurretIO::ConfigureDevices() {
  ...
  const units::turn_t forwardLimit =
      units::turn_t{kMaxAngle / (2.0 * std::numbers::pi * 1_rad) * 1_tr} * kGearRatio;
  const units::turn_t reverseLimit =
      units::turn_t{kMinAngle / (2.0 * std::numbers::pi * 1_rad) * 1_tr} * kGearRatio;
...
void CTRETurretIO::UpdateInputs(TurretIOInputs& inputs) {
  BaseStatusSignal::RefreshAll(
      m_positionSignal, m_velocitySignal, m_accelerationSignal,
      m_currentSignal, m_torqueCurrentSignal, m_tempSignal);
...
void CTRETurretIO::SetTurretAngle(units::radian_t angle) {
  ...
  const units::turn_t mechanismTurns{angle / (2.0 * std::numbers::pi * 1_rad) * 1_tr};
...
void CTRETurretIO::SetBrakeMode(bool brake) {
  ...
  m_motorConfig.MotorOutput.NeutralMode = brake
      ? signals::NeutralModeValue::Brake
      : signals::NeutralModeValue::Coast;
  m_motor.GetConfigurator().Apply(m_motorConfig);
}

void CTRETurretIO::ZeroPosition() {
