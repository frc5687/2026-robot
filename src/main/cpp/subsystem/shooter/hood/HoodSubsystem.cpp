...
void HoodSubsystem::SetHoodPosition(units::turn_t leftHoodRotation, units::turn_t rightHoodRotation){
    ...
    m_leftDesiredSetpoint = leftHoodRotation;
    m_rightDesiredSetpoint = rightHoodRotation;
}

void HoodSubsystem::LogTelemetry(){
    Log("left hood desired setpoint", m_leftDesiredSetpoint.value());
    Log("right hood desired setpoint", m_rightDesiredSetpoint.value());

    Log("left hood Rotation", m_inputs.leftHoodRotation.value());
    Log("right hood rotation rotation", m_inputs.rightHoodRotation.value());
