...
frc::Pose3d
RobotViz::CalculateTurretPoseLeftRobotFrame(const frc::Pose2d &drivePose,
...
frc::Pose3d
RobotViz::CalculateHoodPoseLeftRobotFrame(const HoodState &hoodState) {
  return {Constants::Geometry::kRobotToHoodLeft.Translation(),
          {0_rad, hoodState.angle, 0_rad}};
}
...
void RobotViz::Update() {
  ...
  auto turretPoseLeft =
      CalculateTurretPoseLeftRobotFrame(driveState.pose, turretState);
  ...
  const std::array<frc::Pose3d, 3> components{turretPoseLeft, hoodPoseLeft,
                                              turretPoseRight};
...
void RobotViz::FutureViz(units::second_t futureDt) {
  ...
  Log("Future/DrivePose", driveState.estimatedPose);
